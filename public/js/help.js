...
function main(){
	...
	function getIdAtPosition(posX, posY){
		if (posX < 0 || posY<0 || posX > gl.canvas.width || posY > gl.canvas.height)
			return -1;
		gl.bindFramebuffer(gl.FRAMEBUFFER, fb);
	...
	gl.canvas.addEventListener('mousemove', (e) =>{
		constrect = canvas.getBoundingClientRect();
	...
	function drawScene(){
		...
		gl.uniform3fv(colorUniformLocation, [0.0,0.0,0.0]);
		gl.frawArrays(gl.LINES, 0, gridVertices.length);
...
async function init(){
	...
	await utils.loadFiles([shadeDir + 'lambert-vs.glsl', shaderDir + 'lambert-fs.glsl'], function
		)
}
